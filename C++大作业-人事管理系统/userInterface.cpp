...
{
	//启动界面
	cout << "==========人事管理系统（学校版）==========" << endl;
	cout << "您想要创建新文件还是打开已有文件？" << endl;
	try
	{
...
int userInterface::choose(const list<string>& msg, const string& quit_msg = "退出")
{
	...
	while (!(cin >> choice) || choice<0 || choice>msg.size())
	{
		...
		cerr << "输入必须为不超过" << msg.size() << "的非负整数。请重新输入：";
	}
...
{
	cout << "请输入文件名兼公司名称（可包含路径）。请注意，公司名称设置后不可修改。";
	string filename;
	...
	{
		cerr << "两次输入的密码不一致。" << endl;
		cout << "请重新设置密码（不少于8个字符）：";
...
void userInterface::welcome()
{
	...
	cout << "Creator: 王捷（学号：2022010554）" << endl;
	cout << "公司名称：" << db->getcurrent()->getname() << endl << endl;
}
...
userInterface::interface_type userInterface::view_structure()
{
	...
	else if (choice < all_child.size())//如果选择了某个部门或人员
	{
		...
		{
			cout << "已选择 " << goal_child->getname() << "[部门]" << endl;
...
userInterface::interface_type userInterface::add_new_dept()
{
	...
	while (1)
	{
		...
		{
			cerr << ex.info() << endl;
			cout << db->getcurrent()->getname() << "下已存在部门“" << dept_name << "”。是否重新输入部门名称？" << endl;
...
userInterface::interface_type userInterface::rename_dept()
{
	...
	while (1)
	{
		...
		{
			cerr << ex.info() << endl;
			cout << db->getcurrent()->getname() << "下已存在部门“" << new_dept_name << "”。是否重新输入部门名称？" << endl;
			switch (choose(list<string>{"重新输入部门名称"}, "返回" + db->getcurrent()->getname()))
			{
			case 0:
				cout << "请重新输入新部门的名称：";
...
userInterface::interface_type userInterface::move_dept()
{
	...
	while (1)
	{
		cout << "当前浏览部门：" << db->getcurrent()->getname() << endl;
		...
		else if (choice == dept_num)//若选择移动到该部门
		{
			...
			{
				cerr << ex.info() << endl;
				cout << "部门 " << db->getcurrent()->getname() << " 下已有同名部门，移动部门失败。" << endl;
...
{
	cout << "是否要删除部门 " << db->getcurrent()->getname() << " 及其所有下属部门和人员？" << endl;
